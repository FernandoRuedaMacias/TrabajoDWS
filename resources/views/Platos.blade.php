...
<body>
    ...
    <a href="{{route('inicio')}}"><button type="button" class="block">Volver</button></a>
        ...
        <li><a href="{{route('indiceEmpresas')}}">Empresas</a></li>
   ...
   @csrf
    <table width=100%  class="table table-hover table-dark">
        ...
        <tr scope="row">
            <td>{{$plato->nombre}}</td>
            <td>{{$plato->precio}}</td>
            <td><a href="{{route('almacenarprecio',$plato->precio)}}"><button id="button" class="btn btn-light">Añadir</button></a></td>
        </tr>
        @endforeach
    </table>
    ...
    <div class="center">{{$platos->links()}}</div>


    <div class="center"><a class="btn btn-dark" href="{{route('formcliente')}}">Pagar Pedido</a></div>
